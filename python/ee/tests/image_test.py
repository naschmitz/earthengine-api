...
class ImageTestCase(apitestcase.ApiTestCase):
    ...
    def testConstructors(self):
        ...
        self.assertEqual(ee.ApiFunction.lookup("Image.constant"), from_constant.func)
        ...
        self.assertEqual(
            {"dstImg": ee.Image(1), "srcImg": ee.Image(2)}, from_array.args
        )
        ...
        self.assertEqual({"image": ee.Image(0), "mask": ee.Image(0)}, from_nothing.args)

        from_id_and_version = ee.Image("abcd", 123)
        self.assertEqual(ee.ApiFunction.lookup("Image.load"), from_id_and_version.func)
        ...
        self.assertEqual(
            {"type": "ArgumentRef", "value": "foo"}, from_variable.encode(None)
        )
    ...
    def testCombine(self):
        ...
        self.assertEqual(
            {"dstImg": image1, "srcImg": image2}, combined.args["input"].args
        )
        ...
        self.assertEqual(
            {
                ...
                "values": {
                    ...
                    "1": {
                        "functionInvocationValue": {
                            "arguments": {
                                "expression": {"constantValue": "a"},
                                "argName": {"valueReference": "2"},
                                "vars": {
                                    "arrayValue": {
                                        "values": [
                                            {"valueReference": "2"},
                    ...
                    "2": {"constantValue": "DEFAULT_EXPRESSION_IMAGE"},
                },
            },
...
class CloudThumbnailAndExportImageTests(apitestcase.ApiTestCase):
    ...
    def setUp(self):  # pylint: disable=g-missing-super-call
        ...
        self.cloud_api_resource.projects().thumbnails().create().execute.return_value = {
            "name": "thumbName"
        }
    ...
    def testThumb_withDimensionsRegionCrs(self):
        ...
        with apitestcase.UsingCloudApi(cloud_api_resource=self.cloud_api_resource):
            ...
            self.assertEqual(
                "None/%s/thumbName:getPixels" % _cloud_api_utils.VERSION, url
            )
            _, kwargs = self.cloud_api_resource.projects().thumbnails().create.call_args
            self.assertEqual(
    ...
    def testThumb_withVisualizationParams(self):
        with apitestcase.UsingCloudApi(cloud_api_resource=self.cloud_api_resource):
            self.base_image.getThumbURL(
                {"dimensions": [13, 42], "region": self.geo_json, "min": 0}
            )
            _, kwargs = self.cloud_api_resource.projects().thumbnails().create.call_args
    ...
    def testBuildDownloadIdImage_handlesDimensionsAndScale(self):
        ...
        params = {
            "format": "ZIPPED_GEO_TIFF_PER_BAND",
            "bands": [{"id": "B1", "dimensions": dimensions}],
            "scale": scale,
        }
        image_str = test_image._build_download_id_image(params).serialize()
        self.assertEqual(1, image_str.count(str(dimensions)))
    ...
    def testPrepareForExport_withCrsAndTransform(self):
        with apitestcase.UsingCloudApi():
            ...
            self.assertImageEqual(
                self.base_image.reproject(crs="ABCD", crsTransform=[1, 2, 3, 4, 5, 6]),
                image,
            )
        ...
        with apitestcase.UsingCloudApi():
            ...
            self.assertImageEqual(
                projected.clipToBoundsAndScale(
                    width=3, height=2, geometry=expected_polygon
                ),
                image,
            )
            self.assertEqual({"something": "else"}, params)
    ...
    def testPrepareForExport_withOnlyRegion(self):
        with apitestcase.UsingCloudApi():
            ...
            self.assertEqual(self.base_image.clip(polygon), image)
            self.assertEqual({"something": "else"}, params)
    ...
    def testSelectOptParams(self):
        ...
        self.assertIn('"bandSelectors": {"constantValue": ["selector_a", 4]}', result)
