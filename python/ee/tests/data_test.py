...
class DataTest(unittest.TestCase):
    ...
    def testStartIngestion(self):
        ...
        with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
            ...
            result = ee.data.startIngestion("request_id", manifest, True)
    ...
    def testSetAssetProperties(self):
        ...
        with apitestcase.UsingCloudApi(mock_http=mock_http), mock.patch.object(
            ...
            self.assertSetEqual(
                set(update_mask),
                set(['properties."mYPropErTy"', 'properties."system:time_start"']),
            )
    ...
    def testListAssets(self):
        ...
        with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
            ...
            cloud_api_resource.projects().assets().listAssets().execute.assert_called_once()
            self.assertEqual(mock_result, actual_result)

    def testListAssetsWithPageSize(self):
        mock_http = mock.MagicMock(httplib2.Http)
        ok_resp = httplib2.Response({"status": 200})
        page = b'{"assets": [{"path": "id1", "type": "type1"}], "nextPageToken": "t1"}'
    ...
    def testListAssetsMultiplePages(self):
        mock_http = mock.MagicMock(httplib2.Http)
        ok_resp = httplib2.Response({"status": 200})
        page1 = b'{"assets": [{"path": "id1", "type": "type1"}], "nextPageToken": "t1"}'
        page2 = b'{"assets": [{"path": "id2", "type": "type2"}], "nextPageToken": "t2"}'
    ...
    def testListImages(self):
        ...
        with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
            ...
            cloud_api_resource.projects().assets().listAssets().execute.assert_called_once()
            self.assertEqual(
                {"images": [{"path": "id1", "type": "type1"}]}, actual_result
            )
    ...
    def testListBuckets(self):
        ...
        with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
            ...
            cloud_api_resource.projects().listAssets().execute.return_value = (
                mock_result
            )
    ...
    def testComplexGetListViaCloudApi(self):
        ...
        with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
            mock_result = {
                "assets": [{"name": "id1", "type": "IMAGE", "size_bytes": 1234}]
            }
            cloud_api_resource.projects().assets().listAssets().execute.return_value = (
                mock_result
            )
            actual_result = ee.data.getList(
                {"id": "glam", "num": 3, "starttime": 3612345, "filter": "foo"}
            )
            expected_params = {
                "parent": "projects/earthengine-public/assets/glam",
                "pageSize": 3,
    ...
    def testGetMapId(self):
        ...
        with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
            ...
            self.assertEqual(
                "projects/earthengine-legacy/maps/DOCID", actual_result["mapid"]
            )
    ...
    def testGetDownloadId(self, _):
        ...
        with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
            ...
            actual_result = ee.data.getDownloadId(
                {"image": image.Image("my-image"), "name": "dummy"}
            )
            cloud_api_resource.projects().thumbnails().create().execute.assert_called_once()
            self.assertEqual(
                {"docid": "projects/earthengine-legacy/thumbnails/DOCID", "token": ""},
                actual_result,
            )
        ...
        with ee.data.workloadTagContext("downloadid-tag"):
            ...
            with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
                ...
                self.assertEqual(
                    ...
                    cloud_api_resource.projects()
                    .thumbnails()
                    .create.call_args.kwargs["workloadTag"],
                )
    ...
    def testGetDownloadId_withImageID(self):
        ...
        with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
            with self.assertRaisesRegex(
                ee.ee_exception.EEException, "^Image ID string is not supported."
            ):
    ...
    def testGetDownloadId_withSerializedImage(self):
        ...
        with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
            with self.assertRaisesRegex(
                ...
                ee.data.getDownloadId(
                    {"image": image.Image("my-image").serialize(), "name": "dummy"}
                )
    ...
    def testGetTableDownloadId(self):
        ...
        with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
            ...
            cloud_api_resource.projects().tables().create().execute.assert_called_once()
            self.assertEqual(
                {"docid": "projects/earthengine-legacy/table/DOCID", "token": ""},
                actual_result,
            )
        ...
        with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
            mock_result = {
                ...
                "features": [
                    {
                        "type": "Feature",
                        "properties": {"baz": "qux", "foo": "bar", "system:index": "0"},
                    }
                ],
            }
            cloud_api_resource.projects().assets().listFeatures().execute.return_value = (
                mock_result
            )
            actual_result = ee.data.listFeatures(
                {
                    "assetId": "users/userfoo/foobar",
                    "region": '{"type":"Polygon","coordinates":[[[-96,42],[-95,42],[-95,43],[-96,43],[-96,42]]]}',
                }
            )
            cloud_api_resource.projects().assets().listFeatures().execute.assert_called_once()
            self.assertEqual(mock_result, actual_result)
    ...
    def testGetFeatureViewTilesKey(self):
        ...
        with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
            ...
            cloud_api_resource.projects().featureView().create().execute.return_value = (
                mock_result
            )
            ...
            cloud_api_resource.projects().featureView().create().execute.assert_called_once()
            expected_keys = [
                "token",
                "formatTileUrl",
            ]
    ...
    def testGetProjectConfig(self) -> None:
        ...
        with apitestcase.UsingCloudApi(cloud_api_resource=cloud_api_resource):
            ...
            cloud_api_resource.projects().getConfig().execute.return_value = mock_result
...
def DoCloudProfileStubHttp(test, expect_profiling):
    ...
    def MockRequest(self, method, uri, data, headers, timeout):
        ...
        test.assertEqual(expect_profiling, ee.data._PROFILE_REQUEST_HEADER in headers)
        response = requests.Response()
        response.status_code = 200
        response._content = '{"data": "dummy_data"}'
        if expect_profiling:
            response.headers[ee.data._PROFILE_RESPONSE_HEADER_LOWERCASE] = (
                "someProfileId"
            )
        return response
